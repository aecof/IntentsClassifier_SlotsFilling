...
from src.utils import clip_pad
...
class VanillaEncoderDecoder(nn.Module):
    ...
    def forward(self, input):
        ...
        start_decode = Variable(torch.LongTensor(
            [[1]*input.size(0)])).to(device).transpose(1, 0)
...
class CRFEncoderDecoder(nn.Module):
    ...
    def _build_features(self, input):

        _, (hidden, cell) = self.encode(input)
        ...
        # return intent, decode_slots.view(input.size(0), self.num_slots, length)
        return intent, decode_slots.view(input.size(0), length, self.num_slots), masks

    def forward(self, input):

        intent, features, masks = self._build_features(input)
        scores, tag_seq = self.crf(features, masks)

        return intent, tag_seq, scores, masks, features
    ...
    def loss(self, features, masks, tags):
