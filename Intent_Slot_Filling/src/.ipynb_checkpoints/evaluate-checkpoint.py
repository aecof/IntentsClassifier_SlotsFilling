...
from models import IntentSlotsClassifier, IntentClassifier, SlotsClassifier
from utils import *
...
def main(args):
    ...
    separate = (args.model_type.lower() != 'joint')
...
def evaluate(model, data_loader, device, criterion, criterion2, model2=None, test=False):
    ...
    for seq, gt_intent, gt_slots in data_loader:
        ...
        print(seq.shape[-1], gt_slots.shape[-1])
        seq = seq.to(device)
        ...
        if test:
            ...
            #logging.debug(slots, targets_slots_val)
